#!/usr/bin/env python3
"""Konecty Meta View: manage form view metadata. Stdlib only."""
...
def _flatten_visuals(visuals, depth=0):
    result = []
    for v in (visuals or []):
        vtype = v.get("type", "unknown")
        if vtype == "visualGroup":
            title = v.get("style", {}).get("title", v.get("label", {}))
            lbl = title.get("pt_BR", title.get("en", "")) if isinstance(title, dict) else str(title)
            result.append(("  " * depth + f"[group] {lbl}", vtype))
            result.extend(_flatten_visuals(v.get("visuals", []), depth + 1))
        elif vtype == "visualSymlink":
            fname = v.get("fieldName", "?")
            style_info = ""
            if v.get("style", {}).get("readOnlyVersion"): style_info = " (readonly)"
            if v.get("style", {}).get("renderAs"): style_info += f" renderAs={v['style']['renderAs']}"
            result.append(("  " * depth + f"{fname}{style_info}", vtype))
        elif vtype == "reverseLookup":
            doc = v.get("document", "?")
            field = v.get("field", "?")
            result.append(("  " * depth + f"[reverseLookup] {doc}.{field} → list:{v.get('list', 'Default')}", vtype))
        else:
            result.append(("  " * depth + f"[{vtype}]", vtype))
    return result


def cmd_show(args):
    host, token = _creds(args)
    result = _api(host, token, "GET", f"/{args.document}/view/{args.name}")
...
def cmd_upsert(args):
    ...
    _api(host, token, "PUT", f"/{args.document}/view/{args.name}", doc)
    print(f"View {args.document}:view:{args.name} upserted")


def main():
    parser = argparse.ArgumentParser(description="Konecty Meta View")
    parser.add_argument("--host", default=""); parser.add_argument("--token", default="")
    sub = parser.add_subparsers(dest="command", required=True)
    for name, fn in [("show", cmd_show), ("visuals", cmd_visuals)]:
