...
| **401 / UNAUTHORIZED** on `konecty-admin` tools | OAuth access token expired/invalid and refresh failed | "O acesso admin expirou." | Re-authenticate in Claude Code (`/mcp` → `konecty-admin` → reconnect, browser login again). |
| **Consent screen missing** — browser OAuth redirect dead-ends / blank page | The deployment does not host the consent SPA (`KONECTY_UI_URL` / `UI_PROXY` unset) | "A tela de login/consentimento não está publicada nesse servidor." | Server-side only: ask the company to upgrade/configure the Konecty UI (consent SPA), or — for admin access specifically — provision a trusted client via `OAUTH_CLIENTS_JSON` so the admin flow can proceed once the SPA is fixed. There is no client-side fallback. |
| **Admin option not showing at consent** (OAuth admin path) | The OAuth client is not a trusted client with `admin` allowed, or the user is not `admin: true`. DCR-registered clients are never trusted | "O escopo admin só aparece para clientes confiáveis e usuários admin." | The deployment seeds a trusted client via `OAUTH_CLIENTS_JSON` (`trustedFirstParty: true`, `admin` in `allowedScopes`, exact localhost redirect) — recipe in Konecty `docs/en/mcp.md` (Admin MCP via OAuth section). Then register with `--client-id`/`--callback-port`. |
...
| **App OAuth won't complete** (no browser / headless-like app session) | The `/mcp` → Authenticate UX can't run | "Sem navegador, não é possível completar o login." | No client-side fallback — OAuth needs a browser session. Run setup from an environment with a browser, or ask the company to check the deployment's OAuth/consent setup. |
