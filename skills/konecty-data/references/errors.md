...
| **401 / `UNAUTHORIZED`** (may carry a `WWW-Authenticate` header) | Token missing, expired, or invalid | "Sua sessão expirou — é preciso entrar de novo." | Re-authenticate via OAuth: reconnect/re-authorize the MCP server (konecty-setup "fix auth", [auth.md](auth.md)). |
