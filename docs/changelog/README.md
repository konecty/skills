...
| Date | Summary |
|------|---------|
| [2026-07-14](./2026-07-14_setup-multi-environment-oauth.md) | **Setup OAuth multi-ambiente**: `konecty-setup` deixa de sugerir "só no terminal" — nova matriz *OAuth login by environment* (CLI + app desktop → `/mcp` Authenticate; claude.ai web → claude.ai/customize/connectors; headless → `--no-browser` ou fallback Bearer). Sem dead-end em sessão sem navegador; agente roda `claude mcp add` sozinho; troubleshooting ganha bug do issuer (`/mcp` "Unable to connect" → setar `KONECTY_URL`), web e headless. Docs-only; empilha no #8 |
