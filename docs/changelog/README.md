...
| Date | Summary |
|------|---------|
| [2026-07-11](./2026-07-11_find-via-mcp-impl.md) | `find-via-mcp` implementation: `konecty-data` `find`/`query`/`sql` route through the Konecty **User MCP** (`POST /mcp`, SSE, `Authorization: Bearer`) via a new stdlib `mcp_client.py` + MCP-first dispatcher with automatic REST fallback (`KONECTY_MCP` env: `0`/`only`); output-parity + `_meta` reconstruction; `MockKonecty` `/mcp` SSE route + fault injection; 541 e2e tests, 93% coverage |
