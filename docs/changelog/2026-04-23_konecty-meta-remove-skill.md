...
Retiring a document module (e.g. moving preferences onto `User`) requires deleting **all** related MetaObjects safely, mirroring the confirmation style of `acme-metas` `apply.js` while using the same HTTP admin API as `konecty-meta-sync`.
