...
- **Feature EXECUTED (branch `feat/find-via-mcp`):** `find-via-mcp` — `konecty-data`'s `find`/`query`/`sql` now read through the Konecty **User MCP** tools (`records_find`/`query_json`/`query_sql`) over `POST /mcp` (stateless Streamable HTTP), stdlib only, with automatic REST fallback. Shipped in 10 atomic tasks (T1–T10): new `skills/konecty-data/scripts/mcp_client.py` (bare `tools/call`, SSE parser + defensive JSON path, typed errors `McpHttpError(.status)`/`McpTransportError`/`McpToolError`); `find.py` `_dispatch` fallback matrix (404 silent → REST; 403/429/5xx/conn/timeout/bad-SSE → REST + one-line stderr notice first; 429 disables MCP for the run; 401 + tool-validation surfaced, no fallback) with `KONECTY_MCP` env (`1`/unset MCP-first, `0` REST-only, `only` strict); REST bodies kept as `_rest_*` fallbacks; `_reconstruct_query_meta` re-adds `success`+`total` to the bare MCP `meta` for byte-parity. Auth = legacy `authId` as `Authorization: Bearer` (**no OAuth** — ADR-0006); MCP-first + fallback (**ADR-0007**); **nested-filter divergence** accepted + documented (**ADR-0008**; server-side fix doc in the **Konecty repo** at `.specs/quick/002-mcp-nested-filter-divergence/TASK.md`). Test harness: `MockKonecty` `/mcp` SSE route + fault injection; `tests/e2e/test_mcp_client.py` unit suite + dispatcher/find/query/sql mock e2e + coverage closers. **Gate: `make check` green; full e2e 541 passed / 9 skipped (live suite — stack down); coverage 93% (`--fail-under=90`).** Spec/design/tasks: `.specs/features/find-via-mcp/`. **Live finding (unchanged):** `brain-konecty.konecty.dev/mcp` 403s (`mcp_access_denied`) until an admin adds the role to `mcpRoleIds` — the fallback path handles this transparently. **Next:** independent Verifier (orchestrator-run) → PR.
...
### 2026-07-11 — MCP transport convention (feature: find-via-mcp, EXECUTED)

Project-level pattern established while migrating `konecty-data` search to the User MCP; guides any future
MCP migration (e.g. `konecty-meta` → `/admin-mcp`).

- **D1. Auth on `/mcp` = `Authorization: Bearer <authId>`** (the existing first-party token) **+** the same
  token echoed in the tool `authTokenId` argument (defensive against AsyncLocalStorage drop; `resolveToken`
  prefers the arg). No OAuth (ADR-0006).
- **D2. MCP-first with automatic REST fallback** (ADR-0007). Namespace `mcpRoleIds` allowlist is deny-by-
  default, so fallback is core, not an edge case. `KONECTY_MCP` env: `1`/unset = MCP-first, `0` = REST-only
  (skip the round-trip where MCP is known-absent, and the ADR-0008 nested-filter workaround), `only` = strict.
- **D3. Fallback matrix** — 404 silent → REST; 403/429/5xx/conn/timeout/bad-SSE → REST + one short stderr
  notice emitted **first**; 429 disables MCP for the rest of the process; 401 + tool-validation errors are
  **surfaced, not** masked by REST; both-fail surfaces the REST error.
- **D4. Transport client is local, not shared.** `mcp_client.py` is konecty-data-only (not in
  `shared-files.txt`) until a second consumer (admin-mcp) exists — avoids the byte-identical burden (YAGNI).
- **D5. Spikes (resolved from the MCP SDK source, no live access):** stateless server dispatches a **bare
  `tools/call`** (no `initialize` handshake; a batched handshake is rejected); response is **SSE** (Konecty
  runs the transport without `enableJsonResponse`); `Accept` must list both `application/json` +
  `text/event-stream`. Client parses SSE with a defensive plain-JSON fallback.
