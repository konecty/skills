...
- **Feature EXECUTED (PR-ready, pending history force-push):** `konecty-dev` — third skill, advisory. `SKILL.md` (lean router, description 912 chars) + 8 references (~2.5k lines, signatures verified against the real SDKs/server) written by parallel Sonnet subagents. Cross-links 100% resolve; `make check` green; `make validate` passes (the `name does not match directory "."` error is a `gh skill` env quirk affecting konecty-data too). Committed: `feat(konecty-dev)` (3c81dd7 lineage). Specs: `.specs/features/konecty-dev/`.
  - **Pending:** `make audit` completion gate; optional triggering eval via `skill-creator` (cross-link/boundary checked manually); **force-push of the rewritten history** (see below).
- **Client-name purge (D13) — done locally, NOT yet pushed.** "acme" was found in 6 pre-existing tracked files (konecty-meta references + 1 changelog) — pre-existing debt, not from konecty-dev (my files were clean). Per user decision (2026-06-17): (1) **working tree anonymized** to generic placeholders (`acme`, `*.example.com`, `Acme Inc.`) — committed `chore(konecty-meta): anonymize` ; (2) **git history rewritten** with `uvx git-filter-repo --replace-text` across ALL local refs → **0 acme blobs in entire history**. Backup bundle: `/tmp/konecty-skills-prefilter-backup.bundle` (pre-rewrite SHAs feat=2793d3c main=9b7e402). **`origin` was removed by filter-repo and re-added.** Remote `origin/*` still holds the OLD history until force-pushed — main, feat/konecty-skills-consolidation, AND the collaborator branch feat/add-skill-code-review all share the acme ancestors and need force-push to fully purge the public repo. Force-push is the user's call (rewrites the public default branch + a collaborator PR branch).
